...
enum MUEvent : uint8_t { // コールバックで通知されるイベント
  ...
  /// 受信完了,データなし
  MU_EVENT_RX_COMPLETE_NODATA,
  /// エラー発生,dataにエラーコード
  MU_EVENT_ERROR,
  /// debug フェーズ変化  
  MU_EVENT_PHASE_CHANGE,
};
...
// コンフィグ
  ...
  bool pushRawData(uint8_t *data, uint8_t len) {
    ...
    while (len--) {          // 長さが0になるまで繰り返す
      ...
      case PHASE_DATA:     // データ部分の読み出し
        ...
        if (length_reported == length) {
          concat_idx=0;
          phase =PHASE_TAIL_HASDATA; // データ後に改行コードのフッタがあるためフッタ検出まで待機
        }
        break;
      case PHASE_TAIL:         // 改行コード検出
      case PHASE_TAIL_HASDATA: // データ後の改行コード検出
        footer_char[concat_idx++] = d;
        if (strlen(footer_char) == 2) {
          if (strncmp(footer_char, "\r\n", 2)==0) {
            ...
            if (phase == PHASE_TAIL_HASDATA) { // データありの場合はデータを通知
              ...
              callback(MU_EVENT_RX_COMPLETE_NODATA, nullptr, 0);
            }
            phase = PHASE_WAIT_HEAD;
            break;
          }
    ...
    if(phase!=last_phase){
      last_phase=phase;
      callback(MU_EVENT_PHASE_CHANGE,(uint8_t*)&phase,1);
    }
    return true;
  }
  /**
  ...
  void moduleReset(){
    ...
    char command2[3]="SR";
    sendCommand(command2, nullptr, 0);
  }
