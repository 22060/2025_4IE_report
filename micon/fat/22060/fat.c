//file一覧を表示させ，選択したファイルが.IMGのときに画像を表示させる機能を追加させました．最初SW4でファイルを選択し，SW5を押すことにより画像を表示させます．
//そして，画像が表示されている状態でSW6を押すとファイル一覧画面に戻ることが出来ます．
//ファイル格納順番はどの順番でも大丈夫です
//font6x8.incにフォントデータが格納されています．
...
void TFT_draw_pic(struct file_t *file)
{
    ...
    for (i = 0; i < (240); i++)
    {
        for (int j = 0; j < (320); j++)
        {
            if (i > h - 1 || j > w - 1)
            {
                framebuf[(0 + i) * 320 + (0 + j)] = _COL_WHITE;
            }
            else
            {
                framebuf[(0 + i) * 320 + (0 + j)] = (file->Data[(i * w + j) * 2 + 3] << 8) | (file->Data[(i * w + j) * 2 + 4]);
            }
        }
    }
    TFT_draw_string(0,230,"pre", _COL_BLACK);
    TFT_draw();
    TFT_clear2();
}
...
void show_file_info(struct file_t *file)
{
    ...
    while (status = get_file_info(file))
    {
        ...
        else
        {
            ...
            TFT_draw_string(16, t*10, file->Filename, _COL_BLACK);
