...
void INT_CMT1_CMI0() //1秒周期でfps(frame per secound,画面更新頻度)更新，frag更新機能,led点滅機能(1 / 2 = 0.5Hz周期)
{
	CMT1.CMCSR.BIT.CMF = 0;		// 割り込み要求をクリア

    lastcount = count; //fpsを保持
    frag  = 1; //main関数で扱うようフラグ
    count = 0; //fpsリセット
    LED5 ^= 1;
}
#pragma interrupt INT_MTU2_CMI1
void INT_MTU2_CMI1() //スピーカー制御用関数．このタイマーのコンペアマッチを変更させることで任意の周波数を作り出せる．
{
...
void INT_MTU23_CMI2() //音楽制御用割り込み．一つの音を生成するたびに，すぴかー用の割り込みとこの割り込みのコンペアマッチを変更させる．これにより，midiに対応した制御が楽になる．
{
    ...
    if(make1s >= 10){
        ...
        if (timing < music4count && is_sound) { //is_soundで無音を制御
            // 音を再生する - 事前計算済みの値を使用
            MTU21.TGRA = freq;
        ...
        } else if (timing < music4count) { //無音時間．
            // 無音期間 - 事前計算済みの値を使用
            MTU23.TGRA = dur;
            MTU23.TCNT = 0; // カウンタクリア
            MTU2.TSTR.BIT.CST3 = 1;  // タイマー開始
...
void INT_MTU2_CMI3() //7seg制御用割り込み．今スピーカーで再生している周波数を出力させる．(ダイナミック点灯)
{
...
void update_obstacles()
{
    ...
    for (i = 0; i < MAX_OBSTACLES; i++)
    {
        if (obstacles[i].active)
        {
            ...
            // 敵が弾を発射する（確率的に）
            if ((count + i * 7) % 10 == 0 && obstacles[i].x > 50 && obstacles[i].x < 300)
            {
            ...
            while(1){
                ...
                while (1){
                    ...
                    if(SW4 == 1){
                        while(SW4 == 1)
                            ;
                        TFT_fill_rect2(80,  40,  160,  80,  _COL_WHITE);
                        TFT_draw_string(90,50,"SW5 to restart",_COL_BLACK);
                        TFT_draw_string(90,60,"SW6 to return game",_COL_BLACK);
                        TFTCTRL = 0x4001;
                        DMA0((void *)framebuf,(void*) 0x08000000,0,1,320 * 180);
                        while(SW6 == 0){
                            if(SW5 == 1){
